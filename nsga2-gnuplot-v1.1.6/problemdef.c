...
/* Test problem for multi-objective area and IR drop optimization of PDN */

#ifdef irdrop1
void test_problem (double *xreal, double *xbin, int **gene, double *obj, double *constr)
{
    double s1, s2; /*,l_i, rho;*/
    int i, l_i, rho;
    s1 = s2 = 0.0;
    /*l_i = .0002;
    rho = .0005 Ohm/sq for Cu;*/
    l_i = 200;
    rho = 500;
    for (i=0; i<nreal/2; i++)  /*s1 denotes sum of IR drop of the neighbouring edges*/ /*s2 denotes sum of area of the neighbouring edges */
    {
        s2 += (l_i*rho*xreal[(nreal/2)+i])/xreal[i]; /*xreal[i] denotes w_i        and        xreal[(nreal/2)+i] denotes I_i*/
        s1 += l_i*xreal[i];
    }
    printf("s1=%f\n",s1);
    printf("s2=%f\n\n",s2);
...
void test_problem (double *xreal, double *xbin, int **gene, double *obj, double *constr)
{
    ...
    s1 = s2 = 0.0;
    /*l_i = .0002;
    rho = .02 Ohm/sq for Intel 180nm technology M6 layer;*/
    l_i = 500;
    rho = 0.02;
    for (i=0; i<nreal/2; i++)  /*s1 denotes sum of IR drop of the neighbouring edges*/ /*s2 denotes sum of area of the neighbouring edges */
    {
...
/* Test problem for multi-objective area and IR drop optimization of PDN for a subcircuit*/
...
void test_problem (double *xreal, double *xbin, int **gene, double *obj, double *constr)
{
    ...
    l_4 = 231.5;
    rho = 0.02;
    s2 = (l_1*rho*xreal[(nreal/2)])/xreal[0] + (l_2*rho*xreal[(nreal/2)+1])/xreal[1] + (l_3*rho*xreal[(nreal/2)+2])/xreal[2] + (l_4*rho*xreal[(nreal/2)+3])/xreal[3];
    /*s1 denotes sum of IR drop of the neighbouring edges*/ /*s2 denotes sum of area of the neighbouring edges */

    s1 = l_1*xreal[0] + l_2*xreal[1] + l_3*xreal[2] + l_4*xreal[3];
    /*xreal[i] denotes w_i        and        xreal[(nreal/2)+i] denotes I_i*/

    /*
    for (i=0; i<nreal/2; i++)  
    {
        s2 += (l_i*rho*xreal[(nreal/2)+i])/xreal[i]; 
        s1 += l_i*xreal[i];
    }*/
    printf("s1=%f\n",s1);
    printf("s2=%f\n\n",s2);
...
/* Test problem for multi-objective area and IR drop optimization of PDN for ibmpg1  subcircuit*/
...
void test_problem (double *xreal, double *xbin, int **gene, double *obj, double *constr)
{
    ...
    l_3 = 122.50;
    rho = 0.02;
    s2 = (l_1*rho*xreal[(nreal/2)])/xreal[0] + (l_2*rho*xreal[(nreal/2)+1])/xreal[1] + (l_3*rho*xreal[(nreal/2)+2])/xreal[2] ;
    /*s2 denotes sum of IR drop of the neighbouring edges*/ /*s1 denotes sum of area of the neighbouring edges */

    s1 = l_1*xreal[0] + l_2*xreal[1] + l_3*xreal[2] ;
    /*xreal[i] denotes w_i        and        xreal[(nreal/2)+i] denotes I_i*/
    ...
    obj[0] = s1;
    obj[1] = s2;
    /*constr[0] = 1000 - xreal[(nreal/2)]/xreal[0];
    constr[1] = 1000 - xreal[(nreal/2)+1]/xreal[1];
    constr[2] = 1000 - xreal[(nreal/2)+2]/xreal[2];
    */
    return;
}
#endif
...
void test_problem (double *xreal, double *xbin, int **gene, double *obj, double *constr)
{
    ...
    l_2 = 1.1783e+04;
    rho = 0.02;
    s2 = (l_1*rho*xreal[(nreal/2)])/xreal[0] + (l_2*rho*xreal[(nreal/2)+1])/xreal[1] ;
    /*s2 denotes sum of IR drop of the neighbouring edges*/ /*s1 denotes sum of area of the neighbouring edges */

    s1 = l_1*xreal[0] + l_2*xreal[1] ;
    /*xreal[i] denotes w_i        and        xreal[(nreal/2)+i] denotes I_i*/
...
void test_problem (double *xreal, double *xbin, int **gene, double *obj, double *constr)
{
    ...
    for (i=0; i<nreal/2; i++)
    {
        s1 += (xreal[(nreal/2)+i]*l_i)/xreal[i];
        s2 += l_i*xreal[i];
    }
    obj[0] = s1;
    obj[1] = s2;
